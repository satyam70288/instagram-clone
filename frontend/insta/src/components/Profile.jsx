  ...
  const folooworUnFollowHandler = async (id) => {
    try {
      ...
      if (res.data.success) {
        ...
        // Update the followers list for userProfile based on the action
        const updatedFollowers = isCurrentlyFollowing
          ? userProfile.followers.filter(followerId => followerId !== user._id) // Unfollow: remove user._id
          : [...userProfile.followers, user._id]; // Follow: add user._id
        ...
        dispatch(setUserProfile(updatedUserProfile)); // Update suggested users state  
                ...
                <span>{userProfile?.username}</span>
                {
                  isLoggedInUserProfile ? (
                    ...
                    isFollowing ? (
                      ...
                      <Button className='bg-[#0095F6] hover:bg-[#3192d2] h-8' onClick={() => folooworUnFollowHandler(userId)}>Follow</Button>
                    )
                  )
                }
              </div>
