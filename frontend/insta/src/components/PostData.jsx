...
const PostData = ({ post }) => {
    ...
    const folooworUnFollowHandler = async (id) => {
        try {
            ...
            if (res.data.success) {
                // Check if the logged-in user is currently following the user profile
                const isCurrentlyFollowing = post.author.followers.includes(user._id);
                console.log("post",post)
                // Determine the updated followers list based on whether the user is currently following the author
                const updatedAuthorFollowers = isCurrentlyFollowing
                    ? post.author.followers.filter((followerId) => followerId !== user._id) // Remove the user from followers
                    : [...post.author.followers, user._id]; // Add the user to followers

                // Create an updated posts array with the modified post's author followers
                ...
                // Dispatch the updated posts array to update the Redux state
            ...
            <div className='flex items-center justify-between'>
                ...
                <Dialog>
                    ...
                    <DialogContent className="flex flex-col items-center text-sm text-center">
                        {
                            post?.author?._id !== user?._id && <Button variant='ghost' className={`cursor-pointer w-fit ${isFollowing ? 'text-red-600' : 'text-blue-500'}  font-bold`} onClick={() => folooworUnFollowHandler(post?.author?._id)}>{isFollowing ? 'Unfollow' : 'Follow'}</Button>
                        }
